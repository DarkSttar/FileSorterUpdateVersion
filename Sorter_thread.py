...
class Sorter:
    def __init__(self, gargage: Path, sorting_path: Path):
        ...
        self.semaphore = Semaphore(value=self.count_cpu)
    ...
    def sorting_thread(self):  
        ...
        for type in REGISTER_EXTENTION['TYPES']:            
            thread = Thread(target=self.sorting, args=(type,))
            thread.start()
            threads.append(thread)
            
        [th.join() for th in threads]
        return 'USE THREADING'
    ...
    def start_sorting(self):
        ...
        self.print_result(self.sorting_thread())
        end_time = time()
